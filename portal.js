    ...
    <div class="portal-inner-container">
        ...
        <div id="games-tab" class="portal-tab-content active">
            ...
            <p style="font-size:12px; color:#8b949e;">
                ...
                <span id="live-hashrate" style="color:var(--green); font-weight:bold;">12.54 GH/s</span>
            </p>

            <div class="terminal-box" id="term-log">
                [SYS] Ready to mine blocks...<br>
            </div>

            <button id="mining-toggle-btn" class="portal-btn" style="background:var(--accent);" onclick="toggleMining()">
                Start Mining Engine
            </button>
        </div>
        ...
        <div id="binary-tab" class="portal-tab-content">
            ...
            <div style="position:relative;">
                ...
                <div id="trade-timer"
                    style="position:absolute; top:10px; right:10px; background:rgba(0,0,0,0.8); padding:5px 10px; border-radius:5px; font-weight:bold; color:var(--accent); display:none;">
                    00:60
                </div>
            </div>

            <div class="trade-controls">
                <button id="call-btn" class="portal-btn" style="background:var(--green);" onclick="placeTrade('BUY')">
                    🟢 BUY
                </button>

                <button id="put-btn" class="portal-btn" style="background:var(--red);" onclick="placeTrade('SELL')">
                    🔴 SELL
                </button>
            </div>

            <p id="trade-status"
                style="margin-top:10px; text-align:center; font-weight:bold; color:var(--accent);"></p>
        </div>

        <div id="buy-tab" class="portal-tab-content">
            <h3>💱 USD to ACAT Swap Pool</h3>

            <input type="number" id="usd-amount" class="portal-input"
                placeholder="Enter USD Amount" oninput="calcTokens()" />

            <p style="margin-bottom:10px;">
                You get:
                <b id="acat-preview" style="color:var(--accent);">0</b> ACAT
            </p>

            <button class="portal-btn" onclick="payWithGateway()">
                Pay via Gateway (USDT)
            </button>

            <p id="buy-pool-error"
                style="color:var(--red); font-size:12px; margin-top:5px; font-weight:bold; text-align:center;"></p>
        </div>

        <div id="refer-tab" class="portal-tab-content">
            <h3>👥 Refer & Earn Network</h3>

            <div class="stat-box" style="margin-top:10px; background:#1f242c;">
                Referral Bonus:
                <span id="refer-earning-view">0</span> ACAT
            </div>

            <input type="text" id="ref-link-field" class="portal-input" readonly onclick="this.select();">

            <button class="portal-btn" style="background:var(--accent);" onclick="copyReferralLink()">
                Copy Referral Link
            </button>

            <div class="referral-box">
                <p style="color:#8b949e; font-size:14px; margin-bottom:5px;">
                    Total Friends Invited:
                    <b id="total-ref-count" style="color:white;">0</b>
                </p>

                <p style="color:#8b949e; font-size:14px; margin-bottom:10px;">
                    Total Income:
                    <b id="total-ref-earnings" style="color:var(--green);">0 ACAT</b>
                </p>

                <hr style="border-color:#30363d; margin-bottom:10px;">

                <p style="color:#2ea44f; font-size:12px; font-family:monospace;">
                    System nodes automatically allocate 10% from friend's active validation speed.
                </p>
            </div>
        </div>
        ...
        <div id="withdraw-tab" class="portal-tab-content">
            ...
            <input type="text" id="wallet-input-field" class="portal-input"
                placeholder="Enter Your BEP20 Wallet Address" />

            <p style="font-size:11px; color:#8b949e; margin-top:-5px; margin-bottom:10px; padding-left:2px;">
                ⚠️ Network Rule: Supported asset path is
                <b>BEP20 (Binance Smart Chain)</b> only.
            </p>

            <input type="number" id="withdraw-amount" class="portal-input"
                placeholder="Minimum 100,000 ACAT" />

            <button class="portal-btn" style="background:var(--red);" onclick="submitWithdraw()">
                Withdraw to BEP20
            </button>
        </div>
    </div>`;
...
if(!userId) {
    userId = localStorage.getItem('acat_web_uid')
        || "web_user_" + Math.floor(100000 + Math.random() * 900000);
...
if(!username) {
    username =
        localStorage.getItem('acat_web_uname')
        || "WebArtist_" + userId.split('_')[2]
        || "Guest";
...
async function deductBalance(amount) {
    ...
    if(userBalance < amount) {
        alert(`❌ Insufficient Balance! You need ${amount.toLocaleString()} ACAT`);
        return false;
    }
...
async function loadUserData(){
    ...
    try{
        ...
        if(data){
            ...
            console.log("✅ 500 ACAT Welcome Bonus Added");
        }
...
async function checkReferralParameters() {
    ...
    if (
        referrerId &&
        referrerId !== userId &&
        !localStorage.getItem('acat_ref_processed')
    ) {
        ...
        try {
            ...
            if(refData) {
                ...
                let curCount =
                    parseInt(refData.referrals_count || 0) + 1;
                ...
                await fetch(`${FIREBASE_URL}/${referrerId}.json`, {
                    ...
                    body: JSON.stringify({
                        referrals_count: curCount,
                        referral_rewards: curRewards,
                        points: curPoints
                    })
                });
...
async function fetchGlobalNetworkCount() {
    ...
    try {
        ...
        if(allUsers) {
            ...
            globalUsersCount =
                Object.keys(allUsers).length + 1000;

            rewardScale = 1000 / globalUsersCount;
...
let flowerPos = "center";
let butterflyY = -40;
let butterflyColumn = "center";
...
function moveFlower(position) {
    ...
    document.getElementById('live-flower').style.left =
        lanes[position];
}
...
function startGame() {
    ...
    gameLoopInterval = setInterval(updateGameFrame, 20);
}
...
function spawnButterfly() {
    ...
    const bEl = document.getElementById('falling-butterfly');

    bEl.style.left = lanes[butterflyColumn];
    bEl.style.top = butterflyY + "px";
}
...
async function gameOver() {
    ...
    document.getElementById('earned-session').innerText =
        `Mined: +${sessionEarnings.toLocaleString()} ACAT`;
...
function toggleMining(){
    ...
    if(!miningInterval) {
        ...
        miningInterval = setInterval(async()=>{
            ...
            let baseMined =
                userBalance > 500000 ? 1 : 2;

            let dynamicMined =
                Math.round(baseMined * rewardScale);
...
function initCandleChart() {
    ...
    if(!chartTimer)
        chartTimer = setInterval(renderCandleFrame, 1000);
}
...
function renderCandleFrame() {
    ...
    if(currentCandle.close < currentCandle.low)
    ...
    if(candleTimeCounter >= 60) {
        ...
        let baseOpen = currentCandle.close;
    ...
    let currentLivePrice =
        parseFloat(currentCandle.close.toFixed(2));

    let minPrice =
        Math.min(...candleBars.map(c => c.low)) - 5;

    let maxPrice =
        Math.max(...candleBars.map(c => c.high)) + 5;
    ...
    for(let i=0; i<candleBars.length; i++) {
        ...
        let bar = candleBars[i];

        let x = i * (barWidth + gap) + 15;

        let yOpen = mapY(bar.open);

        let yClose = mapY(bar.close);

        let yHigh = mapY(bar.high);

        let yLow = mapY(bar.low);

        let isGreen = bar.close >= bar.open;

        ctx.strokeStyle =
            isGreen ? "#2ea44f" : "#da3637";
        ...
        ctx.fillRect(
            ...
            barWidth,
            Math.max(2, Math.abs(yOpen - yClose))
        );
    }
    ...
    let yLive = mapY(currentLivePrice);
...
async function placeTrade(type) {
    ...
    let chosenAmount =
        parseInt(document.getElementById('trade-amount-input').value);

    if(isNaN(chosenAmount) || chosenAmount <= 0) {

        alert("Please type a valid custom amount.");
        return;
    }
    ...
    strikePrice =
        parseFloat(currentLiveCandle.close.toFixed(2));
    ...
    document.getElementById('trade-status').innerText =
        `Trade Active: ${type} at position ${strikePrice}`;
...
async function resolveTrade() {
    ...
    if(won) {
        ...
        document.getElementById('trade-status').innerText =
            `🎉 WIN! Closed at ${finalPrice}. +${payout.toLocaleString()} ACAT credited.`;
        ...
        document.getElementById('trade-status').innerText =
            `❌ LOSE! Closed at ${finalPrice}. -${currentBetCost.toLocaleString()} ACAT absorbed.`;
    }
...
async function payWithGateway(){
    ...
    let boughtTokens = chosenUsd * 5000;
...
async function submitWithdraw(){
    ...
    if(isNaN(amount) || amount < 100000){
        ...
        alert("❌ Error: Minimum is 100,000 ACAT");
        return;
    }
